...
{
    if (velocityY >= 0) return; // only when moving UP
    ...
    if (top_left == '#' || top_mid == '#' || top_right == '#')
    {
        ...
        player_y = (row + 1) * cell_size; // snap just below that block
    }
}
...
check_ceiling_collision(lvl, player_y, velocityY,
                        player_x, cell_size, PlayerWidth);
        PlayerSprite.setPosition(player_x, player_y);
